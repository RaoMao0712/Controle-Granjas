...
def verificar_acesso_cliente(usuario):
    ...
    if usuario["status_assinatura"] == "teste":
        ...
        return (
            usuario["data_fim_teste"]
            and hoje <= usuario["data_fim_teste"]
        )
...
def buscar_media_postura_anterior(
    usuario_id,
    lote
):
    conn = conectar()
    ...
    return round(
        sum(posturas) / len(posturas),
        1
    )


def buscar_historico_por_lote(
    usuario_id,
    lote
):
    conn = conectar()
...
def redefinir_senha(token):
    ...
    if request.method == "POST":
        ...
        nova_senha = request.form[
            "nova_senha"
        ].strip()

        confirmar_senha = request.form[
            "confirmar_senha"
        ].strip()
...
def dashboard():
    ...
    if request.method == "POST":
        ...
        if acao == "salvar":
            ...
            entradas = int(
                request.form.get("entradas") or 0
            )

            saidas = int(
                request.form.get("saidas") or 0
            )
...
@app.route(
    "/admin/cliente/<int:cliente_id>/ativar"
)
@login_obrigatorio
@admin_obrigatorio
...
@app.route(
    "/admin/cliente/<int:cliente_id>/bloquear"
)
@login_obrigatorio
@admin_obrigatorio
